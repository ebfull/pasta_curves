    ...
    macro_rules! deferred_field_tests {
        ($F:ty, $mod:ident, $adversarial_a:expr, $adversarial_b:expr) => {
            mod $mod {
                ...
                /// Verifies the fused mul_accumulate (which mac-s directly
                /// into accumulator limbs) matches eager element multiplication.
                #[test]
                fn fused_mul_matches_eager() {
                    let mut rng = XorShiftRng::from_seed(SEED);
                    for _ in 0..100 {
                        let a = <$F>::random(&mut rng);
                        let b = <$F>::random(&mut rng);
                        ...
                        assert_eq!(
                            <$F>::reduce(acc),
                            a * b,
                            "fused mul_accumulate diverged from eager a * b",
                        );
                    }
                }
